...
SELECT state, SUM(user_id) AS total_user_ids
FROM usersAddress
GROUP BY state;
...
SELECT SUBSTR(phone1, 1, 3) AS area_code, COUNT(phone1) AS count
FROM usersContact
GROUP BY area_code
ORDER BY count DESC
LIMIT 1;
...
SELECT MIN(u.first_name) AS min_first_name, ua.county, COUNT(ua.county) AS user_count
FROM users u
